...
export class SheetEditor extends LitElement {
  ...
  private _onSave(): void {
    ...
    this.dispatchEvent(new CustomEvent('widget-save', {
    ...
    return html`
      <div class="sheet-editor">
        ...
        <div class="editor-tabs">
          <button class="${this._activeSection === 'general' ? 'active' : ''}" @click=${() => { this._activeSection = 'general'; }}>
            General
          </button>
          <button class="${this._activeSection === 'data' ? 'active' : ''}" @click=${() => { this._activeSection = 'data'; }}>
            Data
          </button>
        </div>
        ...
        <div class="editor-body">
          ${this._activeSection === 'general' ? html`
            <div class="form-group">
              ...
              <select @change=${(e: Event) => this._updateForm('type', (e.target as HTMLSelectElement).value)}>
            ...
            <div class="form-group">
              ...
              <input
                ...
                @input=${(e: Event) => this._updateForm('title', (e.target as HTMLInputElement).value)}
                placeholder="Widget title"
              />
            </div>

            ${this._form.type === 'chart' ? html`
              <div class="form-group">
                <label>Chart Type</label>
                <select @change=${(e: Event) => this._updateForm('chartType', (e.target as HTMLSelectElement).value)}>
            ...
            ${this._form.type === 'text' ? html`
              <div class="form-group">
                <label>Content</label>
                <textarea
                  .value=${this._form.textContent}
                  @input=${(e: Event) => this._updateForm('textContent', (e.target as HTMLTextAreaElement).value)}
          ...
          ${this._activeSection === 'data' ? html`
            <div class="form-group">
              <label>Query (Natural Language or SQL)</label>
              <textarea
                .value=${this._form.query}
                @input=${(e: Event) => this._updateForm('query', (e.target as HTMLTextAreaElement).value)}

...
export class QuestionNarrativeGenerator {
  private readonly narrativeGenerator: NarrativeGenerator;
  private readonly embeddingEnabled: boolean;

  constructor(config?: Partial<NarrativeConfig> & { embeddingEnabled?: boolean }) {
    this.narrativeGenerator = new NarrativeGenerator(config);
    this.embeddingEnabled = config?.embeddingEnabled ?? false;
  }

  async generateForResult(
    rows: DataRow[],
    intent: AskIntent,
    shape: ResultShape,
    metricField?: CatalogField | null,
  ...
  generateSimpleNarrative(
    ...
    const metricFormat = metricField?.format;
    const metricLabel = metricField?.label || 'value';

    if (!intent.dimensions?.length) {
      const total = rows[0]?.value;
      return `Total ${metricLabel}: ${formatValue(total, metricFormat)}`;
    }

    const valid = rows.filter((r) => this.hasNumericValue(r.value));
    if (valid.length === 0) return 'No numeric data found.';

    const hasLabels = valid.some((r) => r.label !== undefined && r.label !== null);
    if (!hasLabels) return `Total ${metricLabel}: ${formatValue(valid[0].value, metricFormat)}`;

    const sorted = [...valid].sort((a, b) => Number(b.value) - Number(a.value));
    const top = sorted[0];
    const totalVal = valid.reduce((s, r) => s + Number(r.value), 0);

    return `${top.label} leads with ${formatValue(top.value, metricFormat)} (${formatValue(Number(top.value) / totalVal, 'percent')} of total)`;
  }

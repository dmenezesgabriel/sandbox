...
export class FieldResolver {
  ...
  async resolvePhrase(phrase, roles, clarification = null) {
    ...
    for (const strategy of this.strategies) {
      ...
      if (!result) continue;
      if (result.ambiguous) {
        const clarified = clarification?.slot === 'field' && norm(clarification.phrase) === norm(phrase)
          ? result.fields.find(field => field.id === clarification.fieldId)
          : null;
        if (clarified) return { field: clarified };
        return this.clarify({ slot: 'field', phrase, roles }, `Which field did you mean by "${phrase}"?`, result.fields);
      }
...
export class SqlPlanner {
  ...
  plan(intent) {
    ...
    const whereParts = intent.filters.map(filter => {
      ...
      if (filter.operator === 'IN') return `${expr} IN (${(filter.values || []).map(value => `'${escapeSqlString(value)}'`).join(', ')})`;
    ...
    let sql;
    const diagnostics = this.buildDiagnostics({ intent, baseTable, aliases, from, joinSqls: joins, joinRels: joinPlan.joins, whereParts });
    ...
    if (intent.analysisType === 'share') {
      if (!intent.dimensions.length) return { error: 'I need a dimension to calculate share of total.' };
      ...
      sql = `WITH grouped AS (\n${inner}\n), shares AS (\n  SELECT ${labelExpr} AS label, value, CASE WHEN SUM(value) OVER () = 0 THEN NULL ELSE value / SUM(value) OVER () END AS share\n  FROM grouped\n)\nSELECT label, value, share\nFROM shares${shareValues ? `\nWHERE label IN (${shareValues})` : ''}\nORDER BY value ${intent.sort?.direction || 'DESC'}\nLIMIT ${Number(intent.limit) || this.askConfig.maxRows || 25}`;
    ...
    return { sql, columns: intent.dimensions.length ? ['label', 'value'] : ['value'], metricFormat, diagnostics };
  }
  ...
  buildDiagnostics({ intent, baseTable, aliases, from, joinSqls, joinRels, whereParts }) {
    ...
    if (joinSqls.length) {
      ...
      const baseSql = `SELECT COUNT(*) AS row_count FROM ${from}${baseWhereParts.length ? `\nWHERE ${baseWhereParts.join(' AND ')}` : ''}`;
      const joinedSql = `SELECT COUNT(*) AS row_count FROM ${joinedFrom}${whereParts.length ? `\nWHERE ${whereParts.join(' AND ')}` : ''}`;
    ...
    const timeField = intent.timeField || intent.dimensions?.find(field => field.role === 'time') || intent.dateRange?.field;
    if (timeField) {
      ...
      const tryExpr = !isNativeDate && timeField.parseFormat
      ...
      diagnostics.dateParse = {
        ...
        sql: `SELECT COUNT(*) AS checked_rows, SUM(CASE WHEN ${rawExpr} IS NOT NULL AND ${tryExpr} IS NULL THEN 1 ELSE 0 END) AS dropped_rows FROM ${joinedFrom}${whereParts.length ? `\nWHERE ${whereParts.join(' AND ')}` : ''}`
      };
    }
        ...
        if (cueHasFieldName) {
          ...
          return {
            clarification: {
              ...
              pending: { slot: 'filterField', originalQuestion: null, value: items[0].value, valueNormalized: items[0].normalizedValue, candidates },
              choices: candidates
            }
          };
        }
...
export class QuestionParser {
  ...
  async parse(question, options = {}) {
    ...
    const dimensions = await this.resolveDimensions(q, superlative, timeGrain, isCount, warnings, options);
    if (dimensions.clarification) return this.withOriginalQuestion(dimensions, question);
    const filters = this.filterResolver.resolve(q, options.clarification);
    if (filters.clarification) return this.withOriginalQuestion(filters, question);
    ...
    return {
      intent: {
        question,
        analysisType: share ? 'share' : (comparison ? 'comparison' : (isYoY ? 'yoy' : (finalDimensions.some(d => d.role === 'time') ? 'trend' : (isRanking || finalDimensions.length ? 'ranking' : 'kpi')))),
        metric,
  ...
  unsupportedMetricError(question, unsupportedMetric) {
    ...
    return {
      ...
      suggestions: measures.slice(0, 4).map(f => question.replace(new RegExp(unsupportedMetric, 'ig'), f.label.toLowerCase()))
    };
  }
  ...
  async resolveMetric(q, isCount, superlative) {
    if (!isCount) return superlative?.metric || await this.findBestFieldInText(q, 'measure') || this.getDefaultMetric();
    const countEntity = this.entities().find(entity => entity.terms.some(term => this.termMatcher.patternFromTerm(term)?.test(q)));
    return countEntity ? { kind: 'count_distinct', entity: countEntity, field: countEntity.field, label: countEntity.label } : { kind: 'count_star', label: 'Records' };
  }

  detectChange(q) {
    if (!this.hasTerm(q, 'change')) return null;
    const explicitYears = [...q.matchAll(/\b(19\d{2}|20\d{2})\b/g)].map(match => Number(match[1]));
    if (explicitYears.length >= 2) return { startYear: explicitYears[0], endYear: explicitYears[1] };
  ...
  buildShare(q, filters, dimensions) {
    ...
    const contributionFilter = (filters || []).find(filter => !filter.operator || filter.operator === '=');
  ...
  buildComparison(q, filters, dimensions) {
    ...
    const comparisonFilter = { field: group.field, operator: 'IN', values, score: Math.min(...group.filters.map(filter => filter.score || 0.9)), source: 'comparison_values' };
    const remainingFilters = filters.filter(filter => filter.field.id !== group.field.id);
    const comparisonDimension = dimensions.some(dimension => dimension.id === group.field.id) ? [] : [group.field];
    return { values, dimensions: [...comparisonDimension, ...dimensions], filters: [...remainingFilters, comparisonFilter] };
  }
  ...
  async detectListField(q, options = {}) {
    ...
    if (hintedField) return (await this.resolveFieldPhrase(hintedField, ['dimension'], options.clarification)).field;
  ...
  async detectSuperlative(q, options = {}) {
    ...
    if (!phrase) return null;
    const field = (await this.resolveFieldPhrase(phrase, ['dimension'], options.clarification)).field;
  ...
  terms(group) { return this.termMatcher.terms(group); }
  termAlternation(group) { return this.termMatcher.alternation(group); }
  hasTerm(q, group) { return this.termMatcher.has(q, group); }
}
...
export class CatalogBuilder {
  ...
  async build() {
    ...
    const { relationships, ambiguousRelationships } = this.inferRelationships(fields, tableProfiles);
  ...
  inferRelationships(fields, tableProfiles) {
    ...
    if (explicit.length) return { relationships: explicit.map(r => ({ ...r, confidence: 1, inferred: false })), ambiguousRelationships: [] };
    if (this.askConfig.inferRelationships === false) return { relationships: [], ambiguousRelationships: [] };
    ...
    for (const matches of byColumn.values()) {
      for (let i = 0; i < matches.length; i++) {
        for (let j = i + 1; j < matches.length; j++) {
          ...
          if (!rel) continue;
          if (rel.confidence >= (this.askConfig.relationshipInference?.autoAcceptThreshold || 0.85)) accepted.push(rel);
          else if (rel.confidence >= (this.askConfig.relationshipInference?.ambiguousThreshold || 0.6)) ambiguous.push(rel);
        }
      }
    }
...
export class AskDataEngine {
  constructor(config, duckDBManager) {
    ...
    this.confidenceScorer = new ConfidenceScorer({
      ...
      buildJoinPlan: (baseTable, neededTables) => this.sqlPlanner.buildJoinPlan(baseTable, neededTables)
    });
    ...
    this.questionParser = new QuestionParser({
      ...
      localizedTerms: field => this.localizedTerms(field),
      resolveFieldPhrase: (phrase, roles, clarification) => this.fieldResolver.resolvePhrase(phrase, roles, clarification),
  ...
  async ask(question, options = {}) {
    ...
    if (planned.error) return { ...planned, metrics: { catalogBuildMs: this.metrics.catalogBuildMs, parseMs, totalAskMs: Math.round(performance.now() - totalStarted) } };
    ...
    const validationWarnings = this.resultValidator.validate({ rows, intent: parsed.intent, confidence, diagnostics });
  ...
  async evaluateDiagnostics(planned) {
    ...
    if (fanout?.baseCountSql && fanout?.joinedCountSql) {
      try {
        ...
        const ratio = baseCount > 0 ? joinedCount / baseCount : (joinedCount > 0 ? Infinity : 1);
        Object.assign(fanout, { baseCount, joinedCount, ratio: Number.isFinite(ratio) ? Number(ratio.toFixed(3)) : ratio });
    ...
    const selectivity = diagnostics.filterSelectivity;
    if (selectivity?.unfilteredCountSql && selectivity?.filteredCountSql) {
      try {
        const unfiltered = numberValue((toRows(await this.duckDBManager.query(selectivity.unfilteredCountSql))[0] || {}).row_count);
        const filtered = numberValue((toRows(await this.duckDBManager.query(selectivity.filteredCountSql))[0] || {}).row_count);
        ...
        if (unfiltered && ratio < selectivity.threshold) selectivity.warning = `Filters keep only ${(ratio * 100).toFixed(1)}% of rows (${filtered.toLocaleString()} of ${unfiltered.toLocaleString()}). Results may be sparse.`;
      } catch (err) {
        selectivity.error = String(err);
      }
    }
    const dateParse = diagnostics.dateParse;
    if (dateParse?.sql) {
      try {
        ...
        if (droppedRows > 0) dateParse.warning = `Date parsing dropped ${droppedRows.toLocaleString()} ${dateParse.field} rows.`;
      } catch (err) {
  ...
  describeEvidence(intent) {
    ...
    for (const dimension of intent.dimensions || []) {
      evidence.push({ kind: 'dimension', field: this.displayLabel(dimension), table: dimension.table, column: dimension.column, source: 'resolved_field' });
    }
    for (const filter of intent.filters || []) {
      evidence.push({ kind: 'filter', field: this.displayLabel(filter.field), table: filter.field.table, column: filter.field.column, value: filter.operator === 'IN' ? (filter.values || []).join(', ') : filter.value, source: filter.source || 'resolved_value' });
    }
    if (intent.dateRange?.field) {
      evidence.push({ kind: 'date', field: this.displayLabel(intent.dateRange.field), table: intent.dateRange.field.table, column: intent.dateRange.field.column, source: intent.dateRange.kind || 'date_range' });
    }
    return evidence;
  }
  ...
  describeIntent(intent) {
    ...
    if (intent.analysisType === 'change') return `${this.displayLabel(intent.metric || {})} change from ${intent.change?.startYear} to ${intent.change?.endYear}`;
    if (intent.analysisType === 'share') return `Share of ${intent.metric?.aggregation || 'SUM'}(${this.displayLabel(intent.metric || {})}) by ${intent.dimensions.map(d => this.displayLabel(d)).join(' and ')}`;
    if (intent.analysisType === 'comparison') {
      const metric = intent.metric?.kind ? this.displayLabel(intent.metric.field || { label: intent.metric.label }) : this.displayLabel(intent.metric || {});
    ...
    const filters = intent.filters.length ? ` where ${intent.filters.map(f => f.operator === 'IN' ? `${this.displayLabel(f.field)} in ${(f.values || []).join(', ')}` : `${this.displayLabel(f.field)} = ${f.value}`).join(' and ')}` : '';

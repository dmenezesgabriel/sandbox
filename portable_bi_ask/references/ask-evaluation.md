...
async () => {
  ...
  for (const t of smoke.tests.filter((t) => t.kind === 'ask')) {
    ...
    if (e.interpretationContains && !r.interpretation.includes(e.interpretationContains))
      failures.push('interpretation');
    if (e.chartType && r.chartType !== e.chartType) failures.push('chartType');
    if (e.confidenceMin !== undefined && !(r.confidence >= e.confidenceMin))
      failures.push('confidence low');
    if (e.confidenceMax !== undefined && !(r.confidence <= e.confidenceMax))
      failures.push('confidence high');
    for (const expectedWarning of e.warningsContain || []) {
      if (!(r.warnings || []).some((warning) => warning.includes(expectedWarning)))
        failures.push('missing warning');
    }
    for (const forbiddenWarning of e.warningsNotContain || []) {
      if ((r.warnings || []).some((warning) => warning.includes(forbiddenWarning)))
        failures.push('unexpected warning');
    }
    for (const expectedEvidence of e.evidenceContains || []) {
      const found = (r.evidence || []).some((item) =>
        Object.entries(expectedEvidence).every(([key, value]) => item[key] === value),
      );
